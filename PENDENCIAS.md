...
| 4.12 | **Revisar o webhook / deploy automático do Coolify** | você, depois | **Diagnóstico fechado: o problema é só o gatilho.** No push do passo 1.1 (`df5ffc5`) o observador acompanhou **40 minutos com zero trocas de container** — o push não disparou build nenhum. O **Deploy manual funcionou normalmente**: rolling update concluído, container novo de pé em 8 segundos, bundle correspondendo exatamente ao commit. Ou seja, a infraestrutura constrói e sobe bem; o que não funciona é o webhook do GitHub / auto-deploy do recurso. Enquanto não for corrigido, cada deploy exige o clique em *Deploy* — e vale conferir antes de um momento em que a demora importe. **Confirmado uma segunda vez no passo 1.2 (`fb07715`):** push não disparou nada, Deploy manual construiu em 1m45s e concluiu o rolling update em 6s, com as três migrations aplicadas. Duas ocorrências iguais tornam o diagnóstico conclusivo — não é intermitência. **Terceira ocorrência no passo 1.3 (`4ec9f6e`), idêntica às anteriores: push não disparou nada, Deploy manual construiu e subiu em 2m11s.** | pequeno |
