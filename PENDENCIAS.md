...
| 4.12 | **Revisar o webhook / deploy automático do Coolify** | você, depois | No deploy da Fundação (28/08/2026) o `git push` levou cerca de 30 minutos para virar container, e o push seguinte não subiu dentro de 12 minutos de observação. Não bloqueia nada: o `observar:deploy` mostra quando a troca acontece e o `validar:producao` diz se ficou de pé. Mas convém saber se cada implantação depende de um clique manual **antes** de um momento em que isso importe. | pequeno |
